...
- [x] Build the ingestion pipelines
- [x] Create the Bronze layer
- [x] Build Silver transformations
...
### Third Entry (2026-08-27)
*TL;DR:* The Silver layer is complete. The raw match data has been transformed into clean, structured Parquet datasets for match and ban history.

#### Findings

* The transformation into structured Parquet datasets presented relatively few obstacles. One notable issue was that the ban list in Solo Queue match data only contains champion IDs. To resolve this without introducing an additional ingestion pipeline for the complete Data Dragon dataset, I created a Champion dimension directly from the available match history.
* Both Oracle's Elixir and the Riot API Solo Queue match data already include the patch version for each match. Because of this, the Patch dimension created in the previous iteration is no longer required for the Silver layer.

#### Next Steps

* Build the Gold-layer fact tables containing cumulative champion statistics across patches.
* Begin experimenting with scikit-learn to evaluate the first predictive approaches.

In this iteration, I completed the Silver layer by developing the transformation functions required to convert the raw match data into clean, structured Parquet datasets for match and ban history.
